...
if (isServer) then {
	if (("OMTK_MODULE_DISABLE_PLAYABLE_AI" call BIS_fnc_getParamValue) > 0) then {
		...
		addMissionEventHandler ["HandleDisconnect", {
			...
			*/
			[] call omtk_disable_aiBehaviour;
		}];
	};

	if (("OMTK_MODULE_LIGHT_VERSION" call BIS_fnc_getParamValue) < 1 && ("OMTK_MODULE_DISABLE_PLAYABLE_AI" call BIS_fnc_getParamValue) < 1) then {
		{
